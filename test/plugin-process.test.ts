...
import type { WorkflowContext, WorkflowDefinition } from "@jugyo/duex";
...
export async function execute(ctx, input) {
  ...
  const output = second.invocation.output as {
    first: { pid: number };
    second: { pid: number };
  };
...
test("a plugin in retry_wait does not block another healthy invocation", async (t) => {
  const waitingEntry = await fixture(
    t,
    `export const execute = ctx => ctx.run("wait", () => { throw new Error("secret detail"); }, { retry: { maxAttempts: 2, initialDelayMs: 1000 } });`,
  );
  ...
  t.after(() => harness.close());
  const waitingInvocation = await harness.api.invoke({
    workflow: waiting.name,
  });
  const healthyInvocation = await harness.api.invoke({
    workflow: healthy.name,
  });
...
test("does not automatically persist resolved secrets in inputs, journals, logs, or fixed errors", async (t) => {
  ...
  const entry = await fixture(
    t,
    `export const execute = ctx => ctx.run("secret-error", () => { throw new Error(process.env.TOKEN); }, { retry: { maxAttempts: 1, initialDelayMs: 1 } });`,
  );
  const workflow = defineWorkflow({
    name: "secret-error",
    version: "1",
    run: (context) =>
      runPluginProcess({
        ...
        input: { public: true },
        env: { TOKEN: "TOKEN_REF" },
        secrets: secrets({ TOKEN_REF: secret }),
      }),
  });
  ...
  const recorded = JSON.stringify({
    ...
    journal: harness.api.getJournal(invocation.invocation.id),
    logs,
  });
...
test("passes only declared environment variables and resolves fresh values for every execution", async (t) => {
  const entry = await fixture(
    t,
    `export const execute = async ctx => ctx.run("environment", () => ({ token: process.env.TOKEN ?? null, leaked: process.env.EH_PARENT_SENTINEL ?? null }));`,
  );
  const original = process.env.EH_PARENT_SENTINEL;
  process.env.EH_PARENT_SENTINEL = "parent-secret";
  t.after(async () => {
    if (original === undefined) delete process.env.EH_PARENT_SENTINEL;
    else process.env.EH_PARENT_SENTINEL = original;
  });
  let value = "first";
...
for (const [name, source, code] of [
  ...
  [
    "invalid response",
    `export const execute = () => { process.send({ type: "unknown", secret: process.env.SECRET }); return new Promise(() => {}); };`,
  ...
  test(`${name} becomes a plugin-scoped failure with a fixed diagnostic`, async (t) => {
    ...
    await assert.rejects(
      runPluginProcess({
        ...
        input: null,
        env: { SECRET: "REF" },
        secrets: secrets({ REF: "hidden" }),
        timeoutMs: 1000,
      }),
      (error: unknown) =>
        error instanceof PluginProcessError && error.code === code && !error.message.includes("hidden"),
    );
  });
}
