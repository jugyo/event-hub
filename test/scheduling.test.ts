...
function registration(id: string, kind: "source" | "consumer", trigger: Json): PluginRegistration {
  return {
    id, kind, directory: `/plugins/${id}`, entrypoint: `/plugins/${id}/index.mjs`, active: true,
...
test("registration resync does not duplicate polling or daily schedules", () => {
  ...
  const runtime = new RuntimeApi({ store, registry: new WorkflowRegistry(workflows), now: () => NOW });
  runtime.init();
  const eventStore = new EventStore({ path: ":memory:" });
  eventStore.migrate();
  const plugins = [
    registration("github", "source", { type: "poll", everyMs: 60_000 }),
    registration("digest", "consumer", { type: "daily", at: "09:00", timezone: "Asia/Tokyo" }),
  ];
  const first = syncPluginSchedules({ runtime, store: eventStore, secrets, plugins, sourceWorkflow: "source", dailyConsumerWorkflow: "daily" });
  const second = syncPluginSchedules({ runtime, store: eventStore, secrets, plugins, sourceWorkflow: "source", dailyConsumerWorkflow: "daily" });
...
test("disables schedules for removed plugins and reuses them when plugins return", () => {
  ...
  const eventStore = new EventStore({ path: ":memory:" });
  eventStore.migrate();
  const plugin = registration("digest", "consumer", { type: "daily", at: "09:00", timezone: "Asia/Tokyo" });
  syncPluginSchedules({ runtime, store: eventStore, secrets, plugins: [plugin], dailyConsumerWorkflow: "daily" });
  assert.deepEqual(syncPluginSchedules({ runtime, store: eventStore, secrets, plugins: [], dailyConsumerWorkflow: "daily" }).disabled, ["plugin:consumer:digest"]);
  assert.deepEqual(syncPluginSchedules({ runtime, store: eventStore, secrets, plugins: [plugin], dailyConsumerWorkflow: "daily" }).updated, ["plugin:consumer:digest"]);
...
test("does not disable plugin-prefixed schedules outside the synchronization layer's ownership", () => {
  ...
  const eventStore = new EventStore({ path: ":memory:" });
  eventStore.migrate();
  runtime.createSchedule({ id: "plugin:maintenance", workflow: "source", every: 60_000 });

  assert.deepEqual(
    syncPluginSchedules({ runtime, store: eventStore, secrets, plugins: [], sourceWorkflow: "source", dailyConsumerWorkflow: "daily" }),
...
export async function execute(ctx, input) {
  ...
  const sourceRegistration = { id: "source", kind: "source" as const, directory, entrypoint: sourceEntry,
    manifest: { id: "source", kind: "source", entry: "source.mjs", config: null, env: {}, trigger: { type: "poll", everyMs: 60_000 } } };
  const consumerRegistration = { id: "digest", kind: "consumer" as const, directory, entrypoint: consumerEntry,
    manifest: { id: "digest", kind: "consumer", entry: "consumer.mjs", config: null, env: {}, trigger: { type: "daily", at: "01:02", timezone: "Asia/Tokyo" } } };
  ...
  let clock = NOW;
  const openRuntime = () => {
    const api = new RuntimeApi({ store: new SqliteStore({ path: runtimePath }), registry: new WorkflowRegistry(), now: () => clock });
    api.init();
    return api;
  };
  ...
  assert.deepEqual(eventStore.queryHistory({ from: new Date(NOW).toISOString(), to: new Date(clock + 1).toISOString() }).events.map(({ id }) => id), ["event-1"]);
  ...
  clock += 3 * 60 * 60_000;
  await runtime.tick({ maxRuns: 10 });
  const daily = runtime.listInvocations({ scheduleId: "plugin:consumer:digest" })[0];
  assert.deepEqual(daily.output, {
    count: 1,
    scheduledAt: "2026-03-07T16:02:00.000Z",
    window: { from: "2026-03-06T16:02:00.000Z", to: "2026-03-07T16:02:00.000Z" },
  });
  ...
  assert.deepEqual(syncPluginSchedules({ runtime, store: eventStore, secrets, plugins: [] }).disabled,
    ["plugin:consumer:digest", "plugin:source:source"]);
  syncPluginSchedules({ runtime, store: eventStore, secrets, plugins });
  assert.equal(runtime.listSchedules().length, 2);
  runtime.close();
});
