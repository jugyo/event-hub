...
test("runs paginated GitHub collection, change notifications, and daily history summaries with fakes", async (t) => {
  ...
  await writeFile(
    notifier,
    `import { appendFile } from "node:fs/promises"; await appendFile(process.argv[2], JSON.stringify({ title: process.argv[3], message: process.argv[4] }) + "\\n");\n`,
  );
  ...
  const commits = [
    {
      sha: "aaa",
      html_url: "https://example.test/aaa",
      author: { login: "alice" },
      commit: {
        message: "cherry-picked change",
        author: { date: "2020-01-01T00:00:00.000Z" },
        committer: { date: "2026-09-09T12:00:00.000Z" },
      },
    },
    {
      sha: "bbb",
      html_url: "https://example.test/bbb",
      author: { login: "bob" },
      commit: {
        message: "second change",
        author: { date: "2026-09-09T22:00:00.000Z" },
        committer: { date: "2026-09-09T23:00:00.000Z" },
      },
    },
  ];
    ...
    if (url.pathname.includes(":generateContent")) {
      ...
      response.end(
        JSON.stringify({
          candidates: [
            {
              content: { parts: [{ text: `Summary ${geminiPrompts.length}` }] },
            },
          ],
        }),
      );
  ...
  for (const path of manifests) {
    ...
    if (manifest.kind === "source") {
      manifest.config.repository = "example/project";
      manifest.config.apiBaseUrl = baseUrl;
      manifest.config.perPage = 1;
    } else {
      manifest.config.geminiBaseUrl = baseUrl;
      manifest.config.notification = {
        command: process.execPath,
        args: [notifier, notifications, "{title}", "{message}"],
      };
    }
  ...
  assert.deepEqual(
    discovery.plugins.map(({ id }) => id),
    ["github.change-notification", "github.commits", "github.daily-summary"],
  );
  const secrets = {
    get: (name: string) => (name === "WORK_GITHUB_TOKEN" ? "fake-github-token" : "fake-gemini-key"),
  };
  ...
  const sourceManifest = source.manifest as {
    config: Json;
  ...
  const collected = await pollSource({
    sourceId: source.id,
    store,
    config: sourceManifest.config,
    now: () => NOW,
    poll: (input) =>
      runPluginProcess({
        context,
        entry: pathToFileURL(source.entrypoint).href,
        input: input as unknown as Json,
        env: sourceManifest.env,
        secrets,
      }) as unknown as Promise<SourcePollPage>,
  });
  ...
  assert.deepEqual(
    history.map(({ occurredAt }) => occurredAt),
    ["2026-09-09T12:00:00.000Z", "2026-09-09T23:00:00.000Z"],
  );
  ...
  assert.deepEqual(
    delivered.runs.map(({ workflow, outcome }) => ({ workflow, outcome })),
    [
  ...
  assert.deepEqual(
    history.map(({ id }) => store.getDelivery("github.change-notification", id)?.status),
    ["completed", "completed"],
  );

  const daily = discovery.plugins.find(({ id }) => id === "github.daily-summary")!;
  const dailyManifest = daily.manifest as {
    config: Json;
    env: Record<string, string>;
  };
  const dailyResult = await runConsumerPlugin({
    context,
    store,
    entry: pathToFileURL(daily.entrypoint).href,
    env: dailyManifest.env,
    secrets,
    input: {
      scheduledAt: NOW.toISOString(),
      window: { from: "2026-09-09T00:00:00.000Z", to: NOW.toISOString() },
      config: dailyManifest.config,
    },
  });
  assert.deepEqual((dailyResult as { counts: Json }).counts, {
    day: 2,
    week: 2,
  });
  assert.equal(geminiPrompts.length, 3);
  const sent = (await readFile(notifications, "utf8"))
    .trim()
    .split("\n")
    .map((line) => JSON.parse(line));
  assert.deepEqual(
    sent.map(({ title, message }) => ({ title, message })),
    [
  ...
  const afterRemoval = await discoverAndSyncPlugins({
    projectRoot: root,
    store,
    now: "2026-09-10T01:00:00.000Z",
  });
  assert.deepEqual(
    afterRemoval.plugins.map(({ id }) => id),
    ["github.commits", "github.daily-summary"],
  );
});
