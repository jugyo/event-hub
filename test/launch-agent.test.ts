...
test("generates a secret-free plist for an absolute path with spaces and only replaces the target service", async (t) => {
  ...
  const runCommand = async (path: string, args: string[]) => {
    ...
    return args[0] === "bootout" ? 113 : 0;
  };

  const first = await registerLaunchAgent({
    projectRoot,
    executable,
    nodeExecutable,
    home,
  ...
  const second = await registerLaunchAgent({
    projectRoot,
    executable,
    nodeExecutable,
    home,
...
test("adds --debug to tick ProgramArguments when registering in debug mode", async (t) => {
  ...
  const result = await registerLaunchAgent({
    ...
    executable: join(temporary, "event-hub"),
...
test("removes the plist after bootstrap fails when bootout reports an unregistered service", async (t) => {
  ...
  await assert.rejects(
    registerLaunchAgent({
      ...options,
      runCommand: async (_path, args) => (args[0] === "bootstrap" ? 5 : 113),
    }),
