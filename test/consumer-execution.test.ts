...
import { deliverConsumerEvent, EventStore, runConsumerPlugin, type EventInput } from "../src/index.ts";
...
test("completes or terminally fails each consumer delivery independently", async (t) => {
  ...
  t.after(() => store.close());
  const failedEntry = await plugin(
    t,
    `
export const execute = ctx => ctx.run("notify", () => {
  ...
  assert.ok(
    parsed.some(
      (line) =>
        line.event === "plugin.invocation_failed" &&
        line.pluginId === "consumer-a" &&
        line.invocationId === "consumer-test",
    ),
  );
  assert.ok(
    parsed.some(
      (line) => line.event === "plugin.invocation_completed" && line.pluginId === "consumer-b" && line.events === 1,
    ),
  );
});
...
export const execute = ctx => ctx.run("notify", async () => {
  ...
  await assert.rejects(
    deliverConsumerEvent({
      ...
      env: {},
      secrets,
      retry: { maxAttempts: 2, initialDelayMs: 1_000 },
      now: () => NOW,
      logger,
    }),
  );
  ...
  assert.deepEqual(
    store.listReadyDeliveries("consumer-a", retryAt.toISOString()).map(({ event }) => event.id),
    ["new-event"],
  );
  await deliverConsumerEvent({
    consumerId: "consumer-a",
  ...
  assert.ok(
    logs
      ...
      .some(
        (line) => line.event === "consumer.event_failed" && line.level === "warn" && line.pluginId === "consumer-a",
      ),
  );
  await assert.rejects(
    deliverConsumerEvent({
